...
load("frs.Rda")
itms = mapply(function(i)strsplit(x = names(X_likert),split = "_")[[i]][1],1:NCOL(X_likert)) #names of items/variables
I=NROW(X_likert); J=length(itms)

# Defining IRTree on Likert data
X=X_likert
M = 5; N=M-1
Tm = matrix(c(1,0,0,NA,1,0,1,NA,0,NA,NA,NA,1,1,NA,0,1,1,NA,1),M,N,byrow = TRUE)
rownames(Tm) = paste0(c(1:M));colnames(Tm) = paste0("node",1:N)
print(Tm)

# Fitting IRTree on Likert data
Ydata = irtrees::dendrify(as.matrix(X),Tm)
mod1_glm = glmmTMB::glmmTMB(formula = value ~ 0+item:node+(0+1|person), family = binomial, data = Ydata)

alphas1 = matrix(summary(mod1_glm)$coefficients$cond[,1],J,N)
sigma_eta1 = summary(mod1_glm)$varcor$cond$person 
R_eta1 = attr(x = sigma_eta1,which = 'correlation')
sigma_eta1 = matrix(as.numeric(sigma_eta1),N,N) 
etas1 = matrix(as.numeric(unlist(lme4::ranef(mod1_glm)$cond$person)),I,N)

z_predicted = predict(mod1_glm,type = "response"); z_predicted[z_predicted<0.5]=0;z_predicted[z_predicted>=0.5]=1;
X = rbind(table(Ydata$value),table(z_predicted)); rownames(X)=c("obs","fit");print(X)
roc1 = pROC::roc(Ydata$value,z_predicted); pROC::auc(roc1)

# Get fuzzy numbers (fuzzy-IRTree)
fuzzyx = getFuzzyNumbers(Eta = etas1,Alpha = alphas1,Tm = Tm)
Y_out_firtree = fuzzyx$Yfuzzy

# Computing support lengths for each beta fuzzy number
xsup=seq(0,1,length.out=101); suppx = rep(NA,I*J); k=0
for(i in 1:I){
  for(j in 1:J){
    k=k+1
    mux = beta_fn(xsup,Y_out_firtree$mode[Y_out_firtree$sbj==i&Y_out_firtree$itm==j],Y_out_firtree$precision[Y_out_firtree$sbj==i&Y_out_firtree$itm==j])
    suppx[k] = max(xsup[mux<=0.1 & mux>0.05])-min(xsup[mux<=0.1 & mux>0.05])
  }
}
Y_out_firtree$supp = suppx

# Computing summary measures (trapezoidal fuzzy numbers)
Y_out_frs = matrix(NA,nrow = I*J,ncol = 6); colnames(Y_out_frs) = c("sbj","itm","mode","supp","cardinality","kauffman")
Y_out_frs[,1:2] = as.matrix(Y_out_firtree[,1:2])
k=0
for(i in 1:I){
  for(j in 1:J){
    k=k+1
    jjd = grep(x = names(X_frs),pattern = itms[j])
    xpar = as.numeric(X_frs[i,jjd])
    Y_out_frs[k,4] = (xpar[length(xpar)]-xpar[1])/100
    if(length(xpar)==4){
      Y_out_frs[k,3] = ((xpar[3]+xpar[2])/2)/100
      Y_out_frs[k,5] = integrate(f = function(x)trapezoidal_fn(x,lb = xpar[1],ub = xpar[4],m1 = xpar[2],m2 = xpar[3]),lower = xpar[1],upper = xpar[4])$value/100
      mux = trapezoidal_fn(lb = xpar[1],ub = xpar[4],m1 = xpar[2],m2 = xpar[3])
      Y_out_frs[k,6] = kaufmann_index(mux)
    }else if(length(xpar)==3){
      Y_out_frs[k,3] = xpar[2]/100
      Y_out_frs[k,5] = integrate(f = function(x)trapezoidal_fn(x,lb = xpar[1],ub = xpar[3],m1 = xpar[2],m2 = xpar[2]),lower = xpar[1],upper = xpar[3])$value/100
      mux = trapezoidal_fn(lb = xpar[1],ub = xpar[3],m1 = xpar[2],m2 = xpar[2])
      Y_out_frs[k,6] = kaufmann_index(mux)
    }
  }
}
Y_out_frs = as.data.frame(Y_out_frs)


# Cardinalities
D1 = cbind(rbind(with(Y_out_firtree,aggregate(cardinality,list(sbj),mean)),
                with(Y_out_frs,aggregate(cardinality,list(sbj),mean))))
D1$type = rep(c("fIrtree","frs"),each=I)
D1 = as.data.frame(D1);names(D1)=c("i","yvar","type")
D1$type = as.factor(D1$type)

# Support lengths
D2 = cbind(rbind(with(Y_out_firtree,aggregate(supp,list(sbj),mean)),
                 with(Y_out_frs,aggregate(supp,list(sbj),mean))))
D2$type = rep(c("fIrtree","frs"),each=I)
D2 = as.data.frame(D2);names(D2)=c("i","yvar","type")
D2$type = as.factor(D2$type)

# Modes
D3 = cbind(rbind(with(Y_out_firtree,aggregate(mode,list(sbj),mean)),
                 with(Y_out_frs,aggregate(mode,list(sbj),mean))))
D3$type = rep(c("fIrtree","frs"),each=I)
D3 = as.data.frame(D3);names(D3)=c("i","yvar","type")
D3$type = as.factor(D3$type)


## Figure 7
#tikz(file='FSS/paper_rev1/cs3_1.tex',width=6.5,height=3.5)
#x11()
par(mfcol=c(1,3))#,mar=rep(1.5,4),mai=c(1.25, 0.6, 1.15, 0.15))
cols=c("peachpuff4","peru")
boxplot(D3$yvar~D3$type,frame=FALSE,names=c("fIRTree","FRS"),col=cols,ylab="",xlab="",main="(A) mode",cex.axis=1.15,cex.main=1.25)
boxplot(D2$yvar~D2$type,frame=FALSE,names=c("fIRTree","FRS"),col=cols,ylab="",xlab="",main="(B) support length",cex.axis=1.15,cex.main=1.25)
boxplot(D1$yvar~D1$type,frame=FALSE,names=c("fIRTree","FRS"),col=cols,ylab="",xlab="",main="(C) cardinality",cex.axis=1.15,cex.main=1.25)
#dev.off()

# Beta linear model for modes
D3$yvar[D3$yvar==1]=1-1e-2; D3$yvar[D3$yvar==0]=1e-2
out = betareg::betareg(formula = yvar~type,data = D3)
summary(out)
#plot(effects::allEffects(out))

# Beta linear model for cardinalities
out = betareg::betareg(formula = yvar~type,data = D1)
summary(out)
#plot(effects::allEffects(out))

# Beta linear model for support lengths
out = betareg::betareg(formula = yvar~type,data = D2)
summary(out)
#plot(effects::allEffects(out))
...
load("dyfrat.Rda")
I=NROW(data_likert); J=NCOL(data_likert)

# Running fuzzy-IRTree on Likert data
X=data_likert
M = 5; N=M-1
Tm = matrix(c(1,0,0,NA,1,0,1,NA,0,NA,NA,NA,1,1,NA,0,1,1,NA,1),M,N,byrow = TRUE)
rownames(Tm) = paste0(c(1:M));colnames(Tm) = paste0("node",1:N)
...
# Computing supports for each fuzzy number
xsup=seq(0,1,length.out=101); suppx = rep(NA,I*J); k=0
for(i in 1:I){
  for(j in 1:J){
    k=k+1
    mux = beta_fn(xsup,Y_out_firtree$mode[Y_out_firtree$sbj==i&Y_out_firtree$itm==j],Y_out_firtree$precision[Y_out_firtree$sbj==i&Y_out_firtree$itm==j])
    suppx[k] = max(xsup[mux<=0.1 & mux>0.005])-min(xsup[mux<=0.1 & mux>0.005])
  }
}
Y_out_firtree$supp = suppx

# Get fuzzy numbers and summary measures (DYFRAT)
Y_out_dyfrat = matrix(NA,nrow = I*J,ncol = 6); colnames(Y_out_dyfrat) = c("sbj","itm","mode","supp","cardinality","kauffman")
Y_out_dyfrat[,1:2] = as.matrix(Y_out_firtree[,1:2])
k=0
xsup = seq(from=0,to=1,length.out=101)
for(i in 1:I){
  for(j in 1:J){
    k=k+1
    jjd = grep(x = names(data_dyfrat),pattern = paste0("DAS_",j))
    xpar = as.numeric(data_dyfrat[i,jjd])
    mux = beta_fn(xsup,xpar[1],xpar[2])
    Y_out_dyfrat[k,4] = max(xsup[mux<=0.1 & mux>0.005])-min(xsup[mux<=0.1 & mux>0.005])
    Y_out_dyfrat[k,3] = xpar[1]
    Y_out_dyfrat[k,5] = integrate(f = function(x)beta_fn(x,xpar[1],xpar[2]),0,1)$value
    Y_out_dyfrat[k,6] = kaufmann_index(mux)
  }
}
Y_out_dyfrat = as.data.frame(Y_out_dyfrat)
Y_out_dyfrat$supp[is.infinite(Y_out_dyfrat$supp)]=NA


# Cardinalities
D1 = cbind(rbind(with(Y_out_firtree,aggregate(cardinality,list(sbj),mean)),
                 with(Y_out_dyfrat,aggregate(cardinality,list(sbj),mean))))
D1$type = rep(c("fIrtree","frs"),each=I)
D1 = as.data.frame(D1);names(D1)=c("i","yvar","type")
D1$type = as.factor(D1$type)
...
# Support lengths
D2 = cbind(rbind(with(Y_out_firtree,aggregate(supp,list(sbj),mean)),
                 with(Y_out_dyfrat,aggregate(supp,list(sbj),mean))))
D2$type = rep(c("fIrtree","frs"),each=I)
D2 = as.data.frame(D2);names(D2)=c("i","yvar","type")
D2$type = as.factor(D2$type)
...
# Modes
D3 = cbind(rbind(with(Y_out_firtree,aggregate(mode,list(sbj),mean)),
                 with(Y_out_dyfrat,aggregate(mode,list(sbj),mean))))
D3$type = rep(c("fIrtree","frs"),each=I)
D3 = as.data.frame(D3);names(D3)=c("i","yvar","type")
D3$type = as.factor(D3$type)

## Figure 8
#tikz(file='FSS/paper_rev1/cs4_1.tex',width=6.5,height=3.5)
#x11()
par(mfcol=c(1,3))#,mar=rep(1.5,4),mai=c(1.25, 0.6, 1.15, 0.15))
cols=c("peachpuff4","peru")
boxplot(D3$yvar~D3$type,frame=FALSE,names=c("fIRTree","DYFRAT"),col=cols,ylab="",xlab="",main="(A) mode",cex.axis=1.15,cex.main=1.25)
boxplot(D2$yvar~D2$type,frame=FALSE,names=c("fIRTree","DYFRAT"),col=cols,ylab="",xlab="",main="(B) support length",cex.axis=1.15,cex.main=1.25)
boxplot(D1$yvar~D1$type,frame=FALSE,names=c("fIRTree","DYFRAT"),col=cols,ylab="",xlab="",main="(C) cardinality",cex.axis=1.15,cex.main=1.25)
#dev.off()
