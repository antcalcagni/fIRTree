...
getFuzzyNumbers = function(Eta=NULL,Alpha=NULL,Tm=NULL,MC=FALSE,K=5000){
  ...
  if(MC==TRUE){
    ...
    YIJ=NULL
    YIJ_stats=matrix(NA,I*J,5); colnames(YIJ_stats) = c("mode","precision","cardinality","centroid","kauffman")
    ...
    YIJ_stats[,5] = mapply(function(i){kaufmann_index(beta_fn(xsup,YIJ_stats[i,1],YIJ_stats[i,2]))},1:(I*J))
  }
